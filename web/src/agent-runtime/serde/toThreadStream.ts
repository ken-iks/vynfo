...
export async function* toThreadStream(
  ...
  for await (const chunk of stream) {
    switch (chunk.body.case) {
      ...
      default:
        throw new Error(
          `unhandled stream chat response body: ${chunk.body.case}`,
        );
    }
  }
}
...
function appendToolCall(
  ...
  const part =
    toolCall.status === ToolCallStatus.DENIED ||
    toolCall.status === ToolCallStatus.ERRORED
      ? { ...next, isError: true }
      : next;

  const existingIndex = content.findIndex(
    (existing) =>
      existing.type === "tool-call" && existing.toolCallId === part.toolCallId,
  );
  if (existingIndex !== -1) {
    return content.map((existing, index) =>
